...
【重要原则 - 关于预约复诊】
1. 你只能查看已有的复诊计划，不能创建新的复诊预约
2. 当用户要求"帮我预约复诊"时，直接回复："请您打电话或微信联系您的主治医生进行预约，我们会根据您的时间安排合适的复诊时间。"
3. 不要说"已经帮您预约了"或"已经记录您的需求"等会误导用户的话
4. 可以建议用户查看小程序中的"复诊"页面，查看已有的复诊计划
...
5. 不替代医生的面诊和治疗建议"""
    ...
    def _get_fallback_response(self, user_message: str, knowledge: Optional[List[str]] = None) -> str:
        ...
        # 关键词匹配回复
        if "预约" in user_message or "挂号" in user_message:
            return "您好，如需预约复诊时间，请您打电话或微信联系您的主治医生进行预约，我们会根据您的时间安排合适的复诊时间。"
        elif "复诊" in user_message:
